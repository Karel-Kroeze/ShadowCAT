...
#' @export
test_must_stop <- function(number_responses, estimate, min_n, max_n, stop_variance_target = NULL, cutoffs = NULL) {
  min_number_items_reached <- is.null(min_n) || number_responses >= min_n
  max_number_items_reached <- number_responses >= max_n
  variance_target_reached <- !is.null(stop_variance_target) && all(diag(attr(estimate, "variance")) < stop_variance_target)
  estimate_far_enough_below_cutoff <- !is.null(cutoffs) && all(estimate + 3 * sqrt(diag(attr(estimate, "variance"))) < cutoffs[number_responses + 1, ])
