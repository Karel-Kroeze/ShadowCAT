...
test_that("estimator is MAP, 3 dimensions, varying number of categories", {
  ...
  initiated_person$administered <- c(6:20, 31:49)
  initiated_person$estimate <- c(2, .3, -1.2)
  attr(initiated_person$estimate, "variance") <- matrix(c(2, 0, 0, 0, 1.2, 0, 0, 0, 1.5), ncol = 3)
...
test_that("estimator is EAP, 3 dimensions, varying number of categories", {
  ...
  expect_equal(round(as.vector(estimated_latent_trait$estimate), 3), c(-.710, -1.572, -.628))
  expect_equal(round(attr(estimated_latent_trait$estimate, "variance"), 3)[1,], c( .077, -.048, -.007))
  expect_equal(round(attr(estimated_latent_trait$estimate, "variance"), 3)[3,], c(-.007, -.219, .243))
})
