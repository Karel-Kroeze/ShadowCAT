...
test_that("number of answers is 5, number of required starting items 5", {
  ...
  item_next <- get_next_item(start_items, information_summary, lp_constraints, lp_characters, estimate, model, answers, prior, available, administered, number_items, number_dimensions, estimator, alpha, beta, guessing, number_itemsteps_per_item, lower_bound, upper_bound)
...
test_that("number of answers is 0, number of required starting items 5, start type random", {
  ...
  item_next <- with_random_seed(2, get_next_item)(start_items, information_summary, lp_constraints, lp_characters, estimate, model, answers, prior, available, administered, number_items, number_dimensions, estimator, alpha, beta, guessing, number_itemsteps_per_item, lower_bound, upper_bound)
...
guessing <- c(rep(.1, number_items / 2), rep(.2, number_items / 2))  
alpha <- matrix(with_random_seed(2, runif)(number_items * number_dimensions, .3, 1.5), nrow = number_items, ncol = number_dimensions)
indeces_shuffled <- with_random_seed(2, sample)(1:50, 50)
alpha[indeces_shuffled[1:10],2:3] <- 0
alpha[indeces_shuffled[11:20],c(1,3)] <- 0
alpha[indeces_shuffled[21:50],1:2] <- 0
beta <- matrix(with_random_seed(2, rnorm)(number_items), nrow = number_items, ncol = 1)
number_itemsteps_per_item <- number_non_missing_cells_per_row(beta)
