...
test_that("stop rule is variance, targets not reached, one dimension", {
  ...
  should_stop <- test_must_stop(number_responses = 15, estimate, min_n = NULL, max_n = 20, stop_variance_target = .2)
...
test_that("stop rule is variance, targets not reached, three dimensions", {
  ...
  should_stop <- test_must_stop(number_responses = 15, estimate, min_n = NULL, max_n = 20, stop_variance_target = c(.2, .2, .2))
  expect_equal(should_stop, FALSE)
})
...
test_that("stop rule is variance, variance reached, three dimensions", {
  ...
  should_stop <- test_must_stop(number_responses = 15, estimate, min_n = 15, max_n = 20, stop_variance_target = c(.2, .2, .2))
  expect_equal(should_stop, TRUE)
})
...
test_that("stop rule is variance, maximum number of items reached, three dimensions", {
  ...
  should_stop <- test_must_stop(number_responses = 20, estimate, min_n = NULL, max_n = 20, stop_variance_target = c(.2, .2, .2))
  expect_equal(should_stop, TRUE)
})
...
test_that("stop rule is variance, variance reached, minimum number of items not reached, three dimensions", {
  estimate <- 1
  attr(estimate, "variance") <- diag(c(.19, .1, .1))
  should_stop <- test_must_stop(number_responses = 15, estimate, min_n = 16, max_n = 20, stop_variance_target = c(.2, .2, .2))
  expect_equal(should_stop, FALSE)
})
...
test_that("stop rule is number of items, target not reached", { 
  ...
  should_stop <- test_must_stop(number_responses = 14, estimate, min_n = NULL, max_n = 15)
  expect_equal(should_stop, FALSE)
})
...
test_that("stop rule is number of items, target reached", {
  estimate <- 1
  attr(estimate, "variance") <- diag(c(.1, .1, .1))
  should_stop <- test_must_stop(number_responses = 15, estimate, min_n = NULL, max_n = 15)
  expect_equal(should_stop, TRUE)
})
...
test_that("stop rule is cut off, not far enough below cut off, one dimension", {
  ...
  should_stop <- test_must_stop(number_responses = 15, estimate, min_n = NULL, max_n = 20, stop_variance_target = .1, cutoffs = cutoffs[,1, drop = FALSE])
  expect_equal(should_stop, FALSE)
})
...
test_that("stop rule is cut off, not far enough below cut off, three dimensions", {
  ...
  should_stop <- test_must_stop(number_responses = 15, estimate, min_n = NULL, max_n = 20, stop_variance_target = .1, cutoffs = cutoffs)
  expect_equal(should_stop, FALSE)
})
...
test_that("stop rule is cut off, far enough below cut off, three dimensions, but minimum number of items not reached", {
  estimate <- c(.5, 1.1, -2.5)
  attr(estimate, "variance") <- diag(c(.4, .1, .2)^2)
  should_stop <- test_must_stop(number_responses = 15, estimate, min_n = 16, max_n = 20, stop_variance_target = .1, cutoffs = cutoffs)
  expect_equal(should_stop, FALSE)
})
...
test_that("stop rule is cut off, not far enough below cut off, three dimensions, but max number of items reached", {
  estimate <- c(.7, 1.1, -2.5)
  attr(estimate, "variance") <- diag(c(.4, .1, .2)^2)
  should_stop <- test_must_stop(number_responses = 15, estimate, min_n = NULL, max_n = 15, stop_variance_target = .1, cutoffs = cutoffs)
  expect_equal(should_stop, TRUE)
})
